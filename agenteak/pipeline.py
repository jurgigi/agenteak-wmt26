...
from .prompts import (FMT_SYSTEM, editor_messages, term_agent_messages,
...
def make_verify(cfg, ctx):
    def verify_node(state: DocState) -> DocState:
        ...
        selected for that chunk. It is the UN-fine-tuned base Latxa-8B-Instruct,
        so it is prompted with few-shot examples (prompts.FEWSHOT_EDITOR); only
        the translator carries adapters. What the editor returns is the
        translation.
        ...
        for n, c in enumerate(mem.chunks, 1):
            ...
            messages = editor_messages(state["domain"], with_terms, src, draft,
                                       terms, few_shot=cfg.editor_few_shot)
            budget = min(cfg.answer_budget,
                         max(96, int(editor.n_tokens(draft or src) * 1.8) + 96))
            edited = clean_output(
                editor.generate(messages, max_new_tokens=budget, temperature=0.2),
